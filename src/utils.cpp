...
int utils::parse_args(int argc, char ** argv, model * pmodel) {
	...
	while (i < argc) {
		...
		if (arg == "-est") {
			...
			model_status = MODEL_STATUS_EST_SH;
		}
		else if (arg == "-esth") {
			model_status = MODEL_STATUS_EST_H_INF;
			string cm = argv[++i];


			if (cm == "doc") {
				pmodel->compare_model = COMPARE_DOC_TOPIC;
				std::cout << cm << endl;
			}
			else if (cm == "word") {
				pmodel->compare_model = COMPARE_WORD_TOPIC;
				std::cout << cm << endl;
			}
		}
		else if (arg == "-esth2") {
			model_status = MODEL_STATUS_EST_H_EST;
			string cm = argv[++i];
		...
		else if (arg == "-hntopics") {
			hDepth = atoi(argv[++i]);
			Kn = (int*)malloc(hDepth*sizeof(int));
	...
	if (model_status == MODEL_STATUS_EST_H_EST) {
		if (dfile == "") {
	...
	if (model_status == MODEL_STATUS_EST_SH) {
		if (dfile == "") {
			printf("Please specify the input data file for model estimation!\n");
			return 1;
		}
	...
	if (model_status == MODEL_STATUS_EST_H_INF) {
		if (dir == "") {
		...
		pmodel->dfile = dfile;
		/*		don't assign K, since K is already defined in existing model
				if (K > 0) {
					pmodel->K = K;
				}*/
...
int utils::read_and_parse(string filename, model * pmodel) {
	...
	while (fgets(buff, BUFF_SIZE_SHORT - 1, fin)) {
		...
		std::cout << optstr << optval << std::endl;
		if (optstr == "alpha") {
